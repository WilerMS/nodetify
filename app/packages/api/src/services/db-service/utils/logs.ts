...
export const DB_LOG_MESSAGES = {
  ...
  CLIENT_TRIGGER_INJECTION_CHECK_ATTEMPT: (conn: DBConnector, table: string) => `[${conn.getClassName()}] - Checking if trigger 'trigger_nodetify_notifications_${table}' exist in database ${conn.config.database} at ${conn.config.host}:${conn.config.port} with ID ${conn.id}.`,
  CLIENT_TRIGGER_INJECTION_CHECK_EXISTS: (conn: DBConnector, table: string) => `[${conn.getClassName()}] - Trigger 'trigger_nodetify_notifications_${table}' already exists in database ${conn.config.database} at ${conn.config.host}:${conn.config.port} with ID ${conn.id}.`,
  CLIENT_TRIGGER_INJECTION_ERROR: (conn: DBConnector) => `[${conn.getClassName()}] - Failed to inject nodetify trigger in database ${conn.config.database} at ${conn.config.host}:${conn.config.port} with ID ${conn.id}.`,
  CLIENT_TRIGGER_INJECTION_SUCCESS: (conn: DBConnector) => `[${conn.getClassName()}] - Trigger successfully injected to database ${conn.config.database} at ${conn.config.host}:${conn.config.port} with ID ${conn.id}.`
}
